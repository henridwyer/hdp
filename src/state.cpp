...
void HDPState::init_hdp_state(double eta, double gamma, double alpha, long size_vocab) {
  eta_ = eta;
  gamma_ = gamma;
  alpha_ = alpha;
...
void HDPState::copy_hdp_state(const HDPState& src_state) {
  ...
  for (long i = 0; i < num_topics_; ++i) {
    memcpy(topic_lambda_[i], src_state.topic_lambda_[i], size_vocab_ * sizeof(long));
  }
...
void HDPState::compact_hdp_state(vct_long* k_to_new_k) {
  long old_num_topics = num_topics_;
...
void HDPState::load_hdp_state(const char* name) {
  ...
  fscanf(info_file, "size_vocab: %ld\n", &size_vocab_);
  fscanf(info_file, "num_topics: %ld\n", &num_topics_);
  fclose(info_file);
  ...
  for (long k = 0; k < num_topics_; ++k) {
    fscanf(topic_count_file, "%ld", &word_counts_by_topic_[k]);
  }
  ...
  for (long k = 0; k < num_topics_; ++k) {
    for (long w = 0; w < size_vocab_; ++w) {
      fscanf(topic_file, "%ld", &topic_lambda_[k][w]);
    }
  }
  fclose(topic_file);
  ...
  for (long k = 0; k < num_topics_; ++k) {
   ...
   fscanf(stick_file, "%ld", &(beta_u_[k]));
  }
  fclose(stick_file);
...
void HDPState::save_hdp_state(const char* name) {
  ...
  fprintf(info_file, "size_vocab: %ld\n", size_vocab_);
  fprintf(info_file, "num_topics: %ld\n", num_topics_);
  fclose(info_file);

  sprintf(filename, "%s.counts", name);
  FILE* topic_count_file = fopen(filename, "w");
  for (long k = 0; k < num_topics_; ++k) {
    fprintf(topic_count_file, "%ld\n", word_counts_by_topic_[k]);
  }
  fclose(topic_count_file);
  ...
  for (long k = 0; k < num_topics_; ++k) {
    fprintf(topic_file, "%ld", topic_lambda_[k][0]);
    for (long w = 1; w < size_vocab_; ++w) {
      fprintf(topic_file, " %ld", topic_lambda_[k][w]);
    }
    fprintf(topic_file, "\n");
  }
  fclose(topic_file);
...
void HDP::init_hdp(double eta, double gamma, double alpha, long size_vocab) {
...
long HDP::iterate_gibbs_state(bool remove, bool permute) {
  if (permute) { // Permute data.
    rshuffle(doc_states_, num_docs_, sizeof(DocState*));
    for (long j = 0; j < num_docs_; ++j)
  ...
  for (long j = 0; j < num_docs_; ++j) {
    ...
    for (long i = 0; i < doc_state->doc_length_; ++i) {
...
long HDP::sample_word_assignment(DocState* doc_state, long i, bool remove, vct* p) {
  long old_k = -1, k;
  if (remove) { 
  ...
  if ((long)p->size() < hdp_state_->num_topics_ + 1) {
    p->resize(2 * hdp_state_->num_topics_ + 1);
  }

  long d = doc_state->doc_id_;
  long w = doc_state->words_[i].word_;

  double p_w = 0.0;
  set<long>::iterator it = unique_topic_by_word_[w].begin(); 
  long j = 0;
...
void HDP::doc_state_update(DocState* doc_state, long i, long update) {
  long d = doc_state->doc_id_;
  long w = doc_state->words_[i].word_;
  long c = doc_state->words_[i].count_; 
  long k = doc_state->words_[i].topic_assignment_;
  ...
  if (update > 0 &&  k == hdp_state_->num_topics_) { // a new topic is generated.
    ...
    if ((long)hdp_state_->word_counts_by_topic_.size() < hdp_state_->num_topics_ + 1) {
      long new_size = 2 * hdp_state_->num_topics_ + 1;
...
void HDP::sample_table_counts(DocState* doc_state, vct* p) {
  long d = doc_state->doc_id_;
  for (long k = 0; k < hdp_state_->num_topics_; ++k) {
    hdp_state_->beta_u_[k] -= table_counts_by_topic_doc_[k][d];
    long n = word_counts_by_topic_doc_[k][d];
    if (n < 2) {
...
void HDP::sample_posterior_sticks() {
  ...
  for (long k = hdp_state_->num_topics_ - 1; k >= 0; --k) {
...
void HDP::compact_hdp_state() {
  long old_num_topics = hdp_state_->num_topics_;
  vct_long k_to_new_k;
  ...
  long new_k, k;
  for (k = 0; k < (long)k_to_new_k.size(); ++k) {
    new_k = k_to_new_k[k];
    if (new_k >= 0) {
      ...
      if (new_k != k) {
        for (long w = 0; w < hdp_state_->size_vocab_; ++w) {
...
double HDP::log_likelihood(const HDPState* old_hdp_state) {
  ...
  for (long i = 0; i < num_docs_; ++i) {
    long d = doc_states_[i]->doc_id_;
  ...
  for (long k = old_num_topics; k < hdp_state_->num_topics_; ++k) {
...
void HDP::save_doc_states(const char* name) {
  ...
  for (long d = 0; d < num_docs_; ++d) {
    fprintf(doc_state_file, "%ld", word_counts_by_topic_doc_[0][d]);
    for (long k = 1; k < hdp_state_->num_topics_; ++k) {
      fprintf(doc_state_file, " %ld", word_counts_by_topic_doc_[k][d]);
    }
...
void HDP::sample_first_level_concentration(double gamma_a, double gamma_b) {
  ...
  unsigned long cc = rbernoulli(pi);
  if (cc == 1)
