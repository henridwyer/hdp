...
unsigned long rmultinomial(const vct& v, double tot) {
...
void choose_k_from_n(long k, long n, long* result, long* src) {
  gsl_ran_choose(RANDOM_NUMBER, (void *) result,  k, (void *) src, n, sizeof(long));
}

void sample_k_from_n(long k, long n, vct_long* result) {
  if ((long)result->size() < k) result->resize(k);
  for (long i = 0; i < k; ++i) {
    result->at(i) = runiform_long(n);
  }
}
